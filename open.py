...
# def openpylxlib(filename, hexdata):
#     wb = Workbook()
#     ws = wb.active
#     row_len = len(hexdata[0])
#     for row in range(1, len(hexdata)+1):
#         for col in range(1, row_len+1):
#             _ = ws.cell(column=col, row=row,
#                         value=hexdata_table[row-1][col-1])
...
def gethexdata(path):
    if os.path.exists(path):
        ...
        return 1
    hexdata_table = []
    for li in range(0, len(hexdata), 318):
        hexdata_col = [hexdata[i:i+2] for i in range(li, li+318, 2)]
        hexdata_table.append(hexdata_col)
    return hexdata_table

def args_parser():
    parser = argparse.ArgumentParser()
    parser.add_argument('--file', action='store', type=str, required=False, help="path to input data file with binary data")
    parser.add_argument('--dir', action='store', type=str, required=False, help="path to directory with binary data files")
    parser.add_argument('--onefile', action='store', type=bool, required=False, help="if flag true reluts saved in one big file")
    parser.add_argument('--format', action='store', type=str, required=False, help=' format for output data file, choose txt csv of xlsx')
